...
(function() {
  var Promise, querystring, slug, _;
  ...
  module.exports = function(api, validation, couchUtils) {
    var auth, conf, couchClient, emptyResolve, getOrCreateAsset, getTeamAssetDetails, handleAddCluster, handleAddData, handleAddUser, handleRemoveCluster, handleRemoveUser, moirai, moiraiConf, teams, users;
    users = api.users;
    teams = api.teams;
    ...
    moirai.getTeamKeys = function(team) {
      ...
      return users.getUsers(couchClient, {
    ...
    handleAddData = function(event, user) {
      if (event.data.publicKeys != null) {
        return teams.getAllTeamRolesForUser(user.name).then(function(teamList) {
          var teamPromises;
